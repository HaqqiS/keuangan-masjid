"use client";
import KategoriForm from "@/app/_components/views/kategori/kategori-form";
import {
...
import { KategoriSkeleton } from "@/app/_components/views/kategori/kategori-skeleton";

interface KategoriClientProps {
  initialKategoris: KategoriType[];
}
...
export default function KategoriClientPage({
  ...
  // Gunakan data dari server sebagai `initialData`
  // Hook ini tidak akan melakukan fetch pada render pertama, tapi akan
  // melakukan refetch saat `filterKategori` berubah.
  const { data: kategoris, isLoading: isLoadingKategoris } =
    api.kategori.getKategori.useQuery(
      { type: filterKategori },
      { initialData: filterKategori === "ALL" ? initialKategoris : undefined },
    );
  ...
  const isProcessing =
    isPendingCreate || isPendingEdit || isPendingDelete || isLoadingKategoris;

  if (isProcessing) return <KategoriSkeleton />;
